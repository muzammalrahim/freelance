...
setShow2(false)
let getid = null
patch(`api/v1/freelancer_profile/${data1?.profile[0]?.id}/`,data1)
.then((response) => {
const data1= response.data1;
var id = JSON.parse(response.data.id);
})
...
let streeet = data1?.profile[0]?.street;
let numberr = data1?.profile[0]?.mobile_no;
...
<div className="ci-account col-md-12">
<p>Address<span>{streeet}</span></p>
<p>Phone<span>{numberr}</span></p>
</div>
<div className="my-profile-links col-md-12">
<a href="">discard changes</a>
<button>save changes</button>
